...
SSL_Backend :: enum
{
	...
	S_Channel        = 8,
	Secure_Transport = 9,
	AX_TLS            = 10, /* never used since 7.63.0 */
	Mbed_TLS          = 11,
	Mesa_Link         = 12,
	Bear_SSL          = 13,
	Rus_TLS           = 14,
...
HTTP_POST_FILENAME :: 1 << 0
HTTP_POST_READFILE :: 1 << 1
HTTP_POST_PTRNAME :: 1 << 2
HTTP_POST_PTRCONTENTS :: 1 << 3
HTTP_POST_BUFFER :: 1 << 4
HTTP_POST_PTRBUFFER :: 1 << 5
HTTP_POST_CALLBACK :: 1 << 6
HTTP_POST_LARGE :: 1 << 7

Http_Post :: struct
{
	next:           ^Http_Post, // next entry in the list
	...
	buffer_length:   i32, // length of buffer field
	content_type:    cstring, // Content-Type
	content_header:  ^curl_slist, // list of extra headers for this form
	more:           ^Http_Post, // if one field name has more than one file
...
Xferinfo_Callback :: #type proc "c" (clientp: rawptr, dltotal: i32, dlnow: i32, ultotal: i32, ulnow: i32) -> i32

Write_Callback :: #type proc "c" (buffer: cstring, size: uint, nitems: uint, outstream: rawptr) -> uint

/* This callback will be called when a new resolver request is made */
Resolver_Start_Callback :: #type proc "c" (resolver_state: rawptr, reserved: rawptr, userdata: rawptr) -> i32
...
FILE_INFO_FLAG_KNOWN_FILENAME :: 1 << 0
FILE_INFO_FLAG_KNOWN_FILETYPE :: 1 << 1
FILE_INFO_FLAG_KNOWN_TIME :: 1 << 2
FILE_INFO_FLAG_KNOWN_PERM :: 1 << 3
FILE_INFO_FLAG_KNOWN_UID :: 1 << 4
FILE_INFO_FLAG_KNOWN_GID :: 1 << 5
FILE_INFO_FLAG_KNOWN_SIZE :: 1 << 6
FILE_INFO_FLAG_KNOWN_HLINKCOUNT :: 1 << 7
...
Chunk_Bgn_Callback :: #type proc "c" (transfer_info: rawptr, ptr: rawptr, remains: i32) -> i64
...
Chunk_End_Callback :: #type proc "c" (ptr: rawptr) -> u32


/* return codes for FNMATCHFUNCTION */
FN_MATCH_FUNC_MATCH :: 0 /* string corresponds to the pattern */
FN_MATCH_FUNC_NOMATCH :: 1 /* pattern doesn't match the string */
FN_MATCH_FUNC_FAIL :: 2 /* an error occurred */


FN_Match_Callback :: #type proc "c" (ptr: rawptr, pattern: cstring, str: cstring) -> i32

SEEK_FUNC_OK := 0
SEEK_FUNC_FAIL := 1 // fail the entire transfer
SEEK_FUNC_CANT_SEEK := 2 // tell libcurl seeking can't be done, so libcurl might try other means instead

Seek_Callback :: #type proc "c" (instream: rawptr, offset: i64, origin: i32) -> i32 // 'curl_off_t' typically translates to 'i64' in Odin

READ_FUNC_ABORT := 0x10000000
READ_FUNC_PAUSE := 0x10000001

TRAILER_FUNC_OK := 0
TRAILER_FUNC_ABORT := 1

Read_Callback :: #type proc "c" (buffer: ^u8, size: uint, nitems: uint, instream: rawptr) -> uint

curl_trailer_callback :: proc "c" (list: ^^curl_slist, userdata: rawptr) -> i32

Sock_Type :: enum
{
	IP_CXN, /* socket created for a specific IP connection */
...
/* The return code from the sockopt_callback can signal information back
   to libcurl: */
SOCK_OPT_OK :: 0
SOCK_OPT_ERROR :: 1 // causes libcurl to abort and return CURLE_ABORTED_BY_CALLBACK
SOCK_OPT_ALREADY_CONNECTED :: 2

Sockopt_Callback :: #type proc "c" (clientp: rawptr, curlfd: i32, purpose: Sock_Type) -> i32
...
Sock_Addr2 :: struct {} // TODO/FIXME: better type??

Open_Socket_Callback :: #type proc "c" (clientp: rawptr, purpose: Sock_Type, address: ^Sock_Addr) -> i32
...
IO_Err :: enum i32
{
	...
	Unknown_CMD, // command was unknown to callback
	Fail_Restart, // failed to restart the read
	Last, // never use
}
...
{
	No_Op, // no operation
	Restart_Read, // restart the read stream from start
	Last, // never use
}

IO_Ctl_Callback :: #type proc "c" (handle: ^Curl, cmd: i32, clientp: rawptr) -> IO_Err
...
Malloc_Callback :: #type proc "c" (size: uint) -> rawptr
Free_Callback :: #type proc "c" (ptr: rawptr)
Realloc_Callback :: #type proc "c" (ptr: rawptr, size: uint) -> rawptr
Strdup_Callback :: #type proc "c" (str: cstring) -> cstring
Calloc_Callback :: #type proc "c" (nmemb: uint, size: uint) -> rawptr
...
Debug_Callback :: proc "c" (handle: ^Curl, type: Info_Type, data: cstring, size: uint, userptr: rawptr) -> i32
...
PREREQ_FUNC_OK := 0 // Return code for when the pre-request callback has terminated without any errors
PREREQ_FUNC_ABORT := 1 // Return code for when the pre-request callback wants to abort the request
...
curl_conv_callback :: proc "c" (buffer: [^]u8, len: uint) -> Code
curl_ssl_ctx_callback :: proc "c" (curl: ^Curl, ssl_ctx: rawptr, userptr: rawptr) -> Code
...
Option :: enum
{
	Write_Data = CURLOPTTYPE_CBPOINT + 1,
...
when ODIN_OS == .Windows && ODIN_ARCH == .amd64 do foreign import libcurl {
	"binaries/libcurl_windows_amd64.lib",
	"system:msvcrt.lib",
	"system:libcmt.lib",
	"system:Ws2_32.lib",
	"system:Wldap32.lib",
...
foreign libcurl
{
	@(link_name="curl_formadd")
	form_add :: proc "c" (httppost: ^^Http_Post, last_post: ^^Http_Post, #c_vararg args: ..any) -> CURLFORMcode ---
	
	@(link_name="curl_formget")
	form_get :: proc "c" (form: ^Http_Post, arg: rawptr, append: curl_formget_callback) -> int ---
	
	@(link_name="curl_formfree")
	form_free :: proc "c" (form: ^Http_Post) ---
	
	@(link_name="curl_getenv")
	get_env :: proc "c" (variable: cstring) -> cstring ---
	
	@(link_name="curl_version")
	version :: proc "c" () -> cstring ---
	
	@(link_name="curl_easy_escape")
	easy_escape :: proc "c" (handle: ^Curl, string: cstring, length: int) -> cstring ---
	
	@(link_name="curl_escape")
	escape :: proc "c" (string: cstring, length: int) -> cstring ---
	
	@(link_name="curl_easy_unescape")
	easy_unescape :: proc "c" (handle: ^Curl, string: cstring, length: int, outlength: ^int) -> cstring ---
	
	@(link_name="curl_unescape")
	...
	@(link_name="curl_global_init_mem")
	global_init_mem :: proc "c" (flags: i64, m: Malloc_Callback, f: Free_Callback, r: Realloc_Callback, s: Strdup_Callback, c: Calloc_Callback) -> Code ---
	
	@(link_name="curl_global_clenup")
	global_cleanup :: proc "c" () ---
	
	@(link_name="curl_global_sslset")
	global_ssl_set :: proc "c" (id: SSL_Backend, name: cstring, avail: ^^^curl_ssl_backend) -> CURLSSLSET ---
	...
	@(link_name="curl_getdate")
	get_date :: proc "c" (p: cstring, unused: ^i64) -> i64 ---
	
	@(link_name="curl_share_init")
	share_init :: proc "c" () -> ^Curl_SH ---
	
	@(link_name="curl_share_setopt")
	share_set_opt :: proc "c" (csh: ^Curl_SH, option: CURLSHoption, #c_vararg _args: ..any) -> CURLSHcode ---
	
	@(link_name="curl_share_cleanup")
	share_cleanup :: proc "c" (csh: ^Curl_SH) -> CURLSHcode ---
	
	@(link_name="curl_version_info")
	version_info :: proc "c" (version: CURLversion) -> ^CURLversion_Info_Data ---
	
	@(link_name="curl_easy_strerror")
	easy_str_error :: proc "c" (code: Code) -> cstring ---
	
	@(link_name="curl_share_strerror")
	share_str_error :: proc "c" (code: CURLSHcode) -> cstring ---
	
	@(link_name="curl_easy_pause")
	easy_pause :: proc "c" (handle: ^Curl, bitmask: int) -> Code ---
	
	@(link_name="curl_easy_init")
	easy_init :: proc "c" () -> ^Curl ---
	
	@(link_name="curl_easy_setopt")
	easy_set_opt :: proc "c" (curl: ^Curl, option: Option, #c_vararg args: ..any) -> Code ---
	...
	easy_get_info :: proc "c" (curl: ^Curl, info: CURLINFO, #c_vararg args: ..any) -> Code ---
	
	@(link_name="curl_easy_duphandle")
	easy_dup_handle :: proc "c" (curl: ^Curl) -> ^Curl ---
	
	@(link_name="curl_easy_reset")
	easy_reset :: proc "c" (curl: ^Curl) ---
	
	@(link_name="curl_easy_recv")
	easy_recv :: proc "c" (curl: ^Curl, buffer: [^]u8, buflen: uint, n: ^uint) -> Code ---
	
	@(link_name="curl_easy_send")
	easy_send :: proc "c" (curl: ^Curl, buffer: [^]u8, buflen: uint, n: ^uint) -> Code ---
	
	@(link_name="curl_easy_upkeep")
